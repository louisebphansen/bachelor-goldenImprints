...
def argument_parser():
    ...
    parser.add_argument('--feature_col_name', trype=str, help='what the name of the new column in the dataset should be')
...
def main():
    ...
    features_train = features_from_dataset(args['train_data'], img_size, model)
    
    # extract features for test dataset
    features_test =  features_from_dataset(args['test_data'], img_size, model)

    # save new train and test-datasets with added embeddings columns
    add_feature_column(args['train_data'], features_train, args['feature_col_name'], args['new_traindata_name'])
    add_feature_column(args['test_data'], features_test, args['feature_col_name'], args['new_testdata_name'])
